...
const config = {
    A187: '👾VARGAS_250BOT👾', // TOBAT SU ASU
...
client.on('qr', qr => {
   ...
   console.log(`[ ${time} ] SCAN QR MU BOT BY VARGAS_250`)
})
		...
		try {
			...
			switch(command) {
				...
				case 'logowolf':
					...
					if (args.length < 1) return reply(`teksnya mana? contoh ${prefix}logowolf VARGAS|Ganss`)
					reply(mess.wait)
           ...
           case 'demote':
              ...
              case 'owner':
               ...
               client.sendMessage(from, '𝐍𝐢𝐞𝐡 𝐧𝐨𝐦𝐞𝐫 *owner* 𝐤𝐮 𝐤𝐚𝐤, 𝐣𝐢𝐤𝐚 𝐦𝐚𝐮 𝐦𝐚𝐬𝐮𝐤𝐚𝐧 aku 𝐤𝐞 𝐠𝐫𝐨𝐮𝐩 𝐤𝐚𝐥𝐢𝐚𝐧 𝐜𝐡𝐚𝐭 𝐝𝐢𝐚 𝐲𝐚𝐡 kak',MessageType.text, { quoted: mek} )
                break  
              ...
              case 'abgjago':
				...
				case 'url2img': 
					...
					if (args.length < 1) return reply('𝐏𝐢𝐥𝐢𝐡 𝐭𝐢𝐩𝐞 𝐧??𝐚 ??𝐚𝐤?')
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `❮ *VARGAS_250* 𝐁𝐑𝐎𝐀𝐃𝐂𝐀𝐒𝐓 ❯\n\n${body.slice(4)}`)
						}
						reply('𝐬𝐮𝐜𝐜𝐬𝐬 𝐛𝐫𝐨𝐚𝐝𝐜𝐚𝐬𝐭')
					}
					break
				...
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply('??𝐚𝐠 𝐭𝐚𝐫𝐠𝐞𝐭 𝐲??𝐧𝐠 𝐦𝐚𝐮 𝐝𝐢 𝐜𝐥𝐨𝐧𝐞!')
